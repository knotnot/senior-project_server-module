...
def topology():
    ...
    server1 = net.addStation(
        'server1', cls=DockerSta, dimage="my_nginx_ml", mac='00:00:00:00:00:13',
    ...
    user1 = net.addStation(
        'user1', cls=DockerSta, dimage="nisach/ddos_attack:v4", mac='00:00:00:00:00:51',
        ip='10.0.0.51/8', range=30, mem_limit='512m', cpu_shares=5, position='50,30,0')
    user2 = net.addStation(
        'user2', cls=DockerSta, dimage="nisach/ddos_attack:v4", mac='00:00:00:00:00:52',
        ip='10.0.0.52/8', range=30, mem_limit='512m', cpu_shares=5, position='10,30,0')

    ap1 = net.addAccessPoint('ap1', ssid='ssid1', mode='g', channel=1, position='30,30,0', range=30, dpid = "1")
    ap2 = net.addAccessPoint('ap2', ssid='ssid1', mode='g', channel=1, position='30,80,0', range=30, dpid = "2")
    ...
    makeTerm(attacker1, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools ethtool iproute2 net-tools -y && iw dev attacker1-wlan0 connect ssid1'")
    time.sleep(10)
    makeTerm(attacker2, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools ethtool iproute2 net-tools -y && iw dev attacker2-wlan0 connect ssid1'")
    time.sleep(10)
    makeTerm(attacker3, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools ethtool iproute2 net-tools -y && iw dev attacker3-wlan0 connect ssid1'")
    time.sleep(10)
    makeTerm(server1, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools iproute2 -y && iw dev server1-wlan0 connect ssid1 && nginx -g 'daemon off;''")
    time.sleep(10)
    makeTerm(server3, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools ethtool iproute2 net-tools -y && iw dev server3-wlan0 connect ssid1 && python3 -m venv venv && source venv/bin/activate && pip install flask && python3 app.py'")
    time.sleep(10)
    makeTerm(server2, cmd="bash -c 'apt-get update && apt-get install iw wireless-tools ethtool iproute2 net-tools -y && iw dev server2-wlan0 connect ssid1 && python3 -m venv venv && source venv/bin/activate && pip install flask && python3 app.py'")

    # Define a custom CLI to add the move and get_position features
    class CustomCLI(CLI):
